...
def main():
    ...
    # Create Sensors
    range_sensor = SatelliteLaserRanging(id="range_sensor", range_error=0, angular_error=0.0, time_error=0)
    radio_sensor = RadiometricTracking(id="radio_sensor", range_error=0, angular_error=0.0, time_error=0)
    optical_sensor = OpticalTracking(id="optical_sensor", range_error=0, angular_error=0.0, time_error=0)
    ...
    predict_sat3 = od_solver.determine_orbit(gs, observables=optical_obs, trajectory=sat0_traj, method="Gauss", plot=True)

    sim.add_satellites({"gibbs_predict": predict_sat1, "lambert_predict": predict_sat2, "gauss_predict": predict_sat3})

    sim.run_all()

    gauss_trajectory = sim.satellite_trajectories["gauss_predict"]
    predict_sat3 = od_solver.determine_orbit(gs, observables=optical_obs, trajectory=gauss_trajectory, method="Gauss")

    sim.plot_all_five(gs_key="NewCastle", sat_keys=["sat0", "gibbs_predict"], step=5)
